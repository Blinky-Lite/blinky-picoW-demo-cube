...
void BlinkyPicoWMqtt::begin(int chattyCathy, int commLEDPin, int resetButtonPin, boolean useFlashStorage, size_t cubeSetting, size_t cubeReading)
{
  ...
  m_cubeHasDataToRead = false;
  m_mqttHasDataToRead = false;
  ...
  {
    Serial.println("");
    Serial.println("Beginning blinkyPicoW");
  ...
  m_pmqttClient->setServer(m_mqttServer.c_str(),m_mqttPort);
  m_pmqttClient->setCallback (mqttSubscribe);
  m_pmqttClient->setKeepAlive(m_mqttKeepAlive);
  m_pmqttClient->setSocketTimeout(m_mqttSocketTimeout);
...
void BlinkyPicoWMqtt::checkMqttConnection() 
{
  ...
  while (itry < MAX_NO_CONNECTION_ATTEMPTS)
  {
    ...
    rp2040.wdt_reset();
    connected = m_pmqttClient->connect(mqttClientId.c_str(),m_mqttUsername.c_str(), m_mqttPassword.c_str());
    rp2040.wdt_reset();
    if (connected) 
    {
      if (m_chattyCathy) Serial.println("    MQTT connected");
      m_pmqttClient->subscribe(m_mqttSubscribeTopic.c_str());
//      m_pmqttClient->publish(m_mqttPublishTopic.c_str(),"hello world");
    ...
    { 
      ...
      int mqttState = m_pmqttClient->state();
      if (m_chattyCathy) 
      {
...
//    return;
  ...
  checkMqttConnection();
  m_pmqttClient->loop();
  if (m_cubeHasDataToRead)
  {
    if (m_chattyCathy) Serial.println("Publishing MQTT data....");
    m_mqttDataHeader.state = 0;
    if(m_initSettings) m_mqttDataHeader.state = 1;
    m_mqttDataHeader.forceArchiveData = 0;
    if(m_forceArchiveData) m_mqttDataHeader.forceArchiveData = 1;
    ...
    if(m_pmqttClient->publish(m_mqttPublishTopic.c_str(), m_pcubeDataSend, numBytes))
    {
      m_cubeHasDataToRead = false;
      m_forceArchiveData = false;
...
boolean BlinkyPicoWMqtt::publishCubeData(uint8_t* pcubeSetting, uint8_t* pcubeReading, boolean forceArchiveData)
{
  ...
  uint8_t* memPtr = m_pcubeDataSend + m_sizeofMqttDataHeader;
  uint8_t* datPtr = pcubeSetting;
  for (int ii = 0; ii < m_sizeofCubeSetting; ++ii)
  {
    *memPtr = *datPtr;
    ++memPtr;
    ++datPtr;
  ...
  datPtr = pcubeReading;
  for (int ii = 0; ii < m_sizeofCubeReading; ++ii)
  {
    *memPtr = *datPtr;
    ++memPtr;
    ++datPtr;
  ...
  m_forceArchiveData = forceArchiveData;
...
void BlinkyPicoWMqtt::subscribeCubeData(char* topic, byte* payload, unsigned int length) 
{
  ...
  uint8_t* memPtr = m_pcubeDataRecv;
  uint8_t* datPtr = (uint8_t*) payload;
  if (m_chattyCathy) 
  {
    Serial.println("");
    Serial.println("Receiving MQTT data....");
  }
  for (int ii = 0; ii < dataLength; ++ii)
  {
    ...
    {
        Serial.print("    ptr: ");
        Serial.print((uint32_t) memPtr);
        Serial.print(" byte: ");
        Serial.println((uint8_t) *datPtr);
    }
    *memPtr = *datPtr;
    ++memPtr;
    ++datPtr;
